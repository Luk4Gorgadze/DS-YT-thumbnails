...
from thumbnail_analysis import (
    perform_thumbnail_analysis,  # Import the new function
)
...
def get_video_ids(youtube, playlist_id, limit=10):
...
def main():
    ...
    # Check if channel_statistics is empty
    if not channel_statistics:
        print("No channel statistics found. Please check the API response.")
        return

    channel_data = pd.DataFrame(channel_statistics)

    # Debugging: Print the channel_data to see its structure
    ...
    try:
        ...
        channel_data['Total_videos'] = pd.to_numeric(
    ...
    # Save to CSV including new fields
